...
export async function verifySbomEvidence(
  ...
  if (sourceType === "image") {
    ...
    if (metadata && Object.prototype.hasOwnProperty.call(metadata, "config")) {
      ...
      if (embeddedConfigSize !== undefined && configBytes.byteLength !== embeddedConfigSize) {
        return blocked("artifact_sbom_binding_mismatch");
      }
    ...
    if (embeddedConfigPayloadSize !== undefined && embeddedConfigSize !== undefined && embeddedConfigPayloadSize !== embeddedConfigSize) {
      return blocked("artifact_sbom_binding_mismatch");
    }
    ...
    if (metadata && Object.prototype.hasOwnProperty.call(metadata, "layers")) {
      ...
      for (let index = 0; index < metadata.layers.length; index += 1) {
        ...
        if (sourceMediaType !== undefined && (typeof sourceMediaType !== "string" || sourceMediaType !== manifestLayer?.mediaType)) {
          return blocked("artifact_sbom_binding_mismatch");
        }
      }
    }
